...
This library supplies five visual forms plus paired comparison bars, drilldowns, and record inspection over two bundled samples. Maps, editing, writeback, forecasting, saved accounts and access control are not implemented. Connectors feed these same components through the prepared-data contract. Add a specialized component only for an actual user task; keep the shared selection/source contract intact.
